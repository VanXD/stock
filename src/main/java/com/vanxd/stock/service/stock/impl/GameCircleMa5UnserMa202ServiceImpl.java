 ...
 */
@Service
public class GameCircleMa5UnserMa202ServiceImpl implements GameService {
    private Logger LOGGER = LoggerFactory.getLogger(GameCircleMa5UnserMa202ServiceImpl.class);
    ...
    private boolean isMa5UnderMa20(List<Day> data, int i, List<Day> buyRecords) {
        ...
        Day beforeYesterday = data.get(i - 2);
        if (!CollectionUtils.isEmpty(buyRecords)
                && buyRecords.get(buyRecords.size() - 1).getDate().until(today.getDate(), ChronoUnit.DAYS) < 5) {
            return false;
        }

        double abs520 = Math.abs(today.getMa10() - today.getMa20());
        boolean isMaPass = today.getMa5() > today.getMa20();
        boolean isLower = yesterday.getMa5() < beforeYesterday.getMa20();
        boolean isMaDirectionPass = today.getMa5() > yesterday.getMa5()
                 && today.getMa10() > yesterday.getMa10()
                && today.getMa20() > yesterday.getMa20();
        return isLower && isMaPass && isMaDirectionPass;
    }
