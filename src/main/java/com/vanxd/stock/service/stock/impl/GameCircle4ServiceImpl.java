 ...
 */
@Service
public class GameCircle4ServiceImpl implements GameService {
    private Logger LOGGER = LoggerFactory.getLogger(GameCircle4ServiceImpl.class);
    ...
    private boolean isHitSatisfied(Day day, List<Long> hit) {
        List<Long> result = hit.stream()
                .filter(item -> item >= (day.getTimestamp() - 2592000000L))
                .collect(Collectors.toList());
        List<Long> maxHitResult = hit.stream()
                .filter(item -> item >= (day.getTimestamp() - 7776000000L))
                .collect(Collectors.toList());
        return result.size() > LEAST_HIT && maxHitResult.size() < MAX_HIT;
