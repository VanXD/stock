...
public class MaxPlusController {
    ...
    private final static String API = "https://q.maxjia.com/api/bets/get_all_category/3/?&bet_type=all&limit=30&offset=50";
    private static AtomicInteger offsetStart = new AtomicInteger(0);

    @GetMapping("/saveMaxPlusData.json")
    public MaxResult saveData() {
        String url = API + offsetStart.getAndAdd(30);
        logger.info("准备请求：{}", url);
        MaxResult maxResult = request(API);
        ...
        for (MaxCategory maxCategory : result) {
            ...
            for (Match match : matches) {
                if (!match.getProgress_desc().equals("已结算")) {
                    continue;
                }
